...
import { Loader, Clipboard, Download, ChevronLeft, ChevronRight, LogOut, Settings } from 'lucide-react';
...
const OdooProductCardGenerator = () => {
  ...
  return (
    <>
      <NavBar />
      <div className="w-screen pt-24 pb-10 px-4 sm:px-6 lg:px-8 bg-gray-50 min-h-screen">
        <header className="w-full bg-white shadow-md py-6 border-b-2 border-indigo-100">
          <div className="w-full max-w-7xl mx-auto px-6">
            <h1 className="bg-white p-6 md:p-10 mx-auto max-w-7xl mb-10 border border-black rounded-xl shadow-2xl">
        ...
        <div className="w-full bg-white border-b border-gray-200">
          <form onSubmit={handleSearchSubmit} className="w-full max-w-7xl mx-auto p-5">
            <input
              ...
              placeholder={`Search (min ${MIN_SEARCH_LENGTH} chars for active search for product (SKU, Barcode, or Name)...`}
              className="p-4 border border-black rounded-xl shadow-inner w-full md:w-1/2 lg:w-1/3 transition-all focus:border-indigo-600 focus:ring-2 focus:ring-indigo-200"
            />
          </form>
        </div>
        ...
        {!isLoading && currentProducts.length > 0 && (
          <>
              ...
              {products.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {products.map((product) => (
                    <ProductCard
                      ...
                      // html2canvasLoaded={html2canvasLoaded} // Removed
                ...
                <div className="text-center p-10 bg-gray-50 border border-black rounded-xl shadow-inner">
                  <p className="text-xl text-gray-700">
                    {isSearchActive
                      ? "No products found matching your search criteria."
                      : "Start typing (min 3 characters) to search for products."}
                  </p>
                </div>
              )}
            ...
            {totalPages > 1 && products.length > 0 && (
              <div className="flex justify-center items-center mt-8 space-x-4">
                <button
                  ...
                  className="p-3 bg-indigo-500 hover:bg-indigo-600 text-white rounded-full disabled:opacity-40 transition-colors border border-black shadow-md"
                >
                  <ChevronLeft className="w-5 h-5" />
                </button>
                <span className="text-gray-800 font-medium">
                    Page {currentPage} of {totalPages} (Showing {products.length} results on this page)
                </span>
                <button
