...
const ProductCard = React.memo(({ product, uniqueKey, copyWhatsAppText, downloadCardAsImage, lastCopiedId, html2canvasLoaded, isDownloadLoading, showToast, templateLines }) => {
  ...
  /*const whatsappText = useMemo(() => {
    let text = templateLines;

    // Replace placeholders with actual product data
    //text = text.replace(/{product_name}/g, product.name || 'N/A');
    //text = text.replace(/{sku}/g, sku);
    //text = text.replace(/{price}/g, formattedPrice);
    //text = text.replace(/{description}/g, description);
    //text = text.replace(/{qty_available}/g, qtyAvailable);
    //text = text.replace(/{brand}/g, sanitizeValue(product.brand) || 'N/A');
    //text = text.replace(/{category}/g, sanitizeValue(product.category) || 'N/A');
    
    let prod_name = text.replace(/{product_name}/g, product.name || 'N/A')
    let prod_sku = text.replace(/{sku}/g, sku)
    let prod_price = text.replace(/{price}/g, formattedPrice)
    let prod_qty = text.replace(/{qty_available}/g, qtyAvailable)
    let prod_descrip = text.replace(/{description}/g, description)
    ...
    const prodValues = [
      ...
      prod_qty,
      prod_descrip
      //text.replace(/{brand}/g, sanitizeValue(product.brand) || 'N/A'),
      //text.replace(/{category}/g, sanitizeValue(product.category) || 'N/A')
    ]; 
    ...
    console.log('templateLines.length is ',templateLines.length)
    console.log('templateLines.length is ',templateLines)
    console.log('templateLines val 1 is ',templateLines[0])
    console.log('templateLines val 2 is ',templateLines[1])
    console.log('templateLines cak 3 is ',templateLines[2])
    console.log('templateLines.val 4 is ',templateLines[3])
    console.log('templateLines.val 5 is ',templateLines[4])
    console.log('templateLines.cal 6 is ',templateLines[5])
    let finalText = '';
    if (templateLines && templateLines.length > 0) {
      for (let i = 0; i < 2; i++) {
        if (i <= 2) {
          finalText += `${templateLines[i]}${prodValues[i]}\n`;
        } else {
          finalText += `${templateLines[i]}\n`;
        }
      }
    }

    return finalText;
  }, [product, sku, formattedPrice, description, qtyAvailable, templateLines]); */
...
export default function OdooProductCardGenerator() {
  ...
  //const [templateLines, setTemplateLines] = useState(localStorage.getItem('whatsappTemplate') || '*{product_name}*\nSKU: {sku}\nPrice: {price}\nAvailable: {qty_available} units\nBrand: {brand}\n\n{description}');
  ...
  /* useEffect(() => {
    // Load html2canvas asynchronously
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (!Array.isArray(data)) {
            ...
            throw new Error("API response was not a list of products.");
        }

        setProducts(data);
        //showToast(`Successfully loaded ${data.length} products.`, 'success');
  ...
  // Sync input state when actual search term changes (e.g., initial load or external reset)
//  useEffect(() => {
//  }, [searchTerm, minQty, maxQty]);
...
//}, [debouncedSearchTerm,searchTerm, minQty, maxQty]);
  ...
  //}, [products, searchTerm, productType, minQty, maxQty]);
...
useEffect(() => {
  ...
  // --- Action Handlers (copy and download remain the same) ---

/* const copyWhatsAppText = useCallback(async (id, text, imageBase64) => {
    let copiedImage = false;
    console.log('Temp. text is :',text)
    try {
