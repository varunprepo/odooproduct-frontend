...
useEffect(() => {
    ...
    const fetchProducts = async () => {
        setIsLoading(true);
        try {
            const response = await fetch(`${API_URL}?search=${encodeURIComponent(searchTerm.trim())}`);
    ...
    const delay = setTimeout(fetchProducts, 400); // debounce input
  ...
  const filteredProducts = useMemo(() => {
    ...
    let list = products.filter(product => {
      ...
      if (searchTerm && searchTerm.length >= MIN_SEARCH_LENGTH) {
        ...
        console.log('The value of searchTerm.length ',searchTerm.length)
      }    
